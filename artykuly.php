...
if(empty($_GET['id']))
	{
		$news=mysql_query("SELECT * FROM arty ORDER BY id desc");
	...
	else
	{
		...
		$ilosc_wierszy=mysql_num_rows($news);
		$id = (int)$_GET['id'];
$sprawdzmy=mysql_query("SELECT * FROM `wizyty` where `id`=$id && gdzie='arty'");
while($linijka=mysql_fetch_array($sprawdzmy)){
if($linijka['ip']!=$_SERVER['REMOTE_ADDR']){
$unikalne=$unikalne+1;}
$ileu=mysql_query("SELECT ileu FROM arty WHERE id=$id");
$unikalne=$unikalne+$ileu['ileu'];
mysql_query("UPDATE arty SET ileu=\"".$unikalne."\" where id=".$id);
}
  if($_SESSION['zalogowany']){
		for($i=1; $i <= $ilosc_wierszy; $i++)
		{
		    ...
		    if($i==$id){
                     ...
                     echo "<br><br><br>";
                     $ilosc=$linia['ile']+1;
                     mysql_query("UPDATE arty SET ile=\"".$ilosc."\" where id=".$linia['id']);
$gdzie='arty';
mysql_query("INSERT INTO wizyty (`id`,`ip`,`gdzie`) VALUES ('".$linia['id']."','".$_SERVER['REMOTE_ADDR']."','".$gdzie."')");
$numerek=$linia['id'];
$wynik=mysql_query("SELECT * FROM komart where id=$linia[id]");
...
echo "<form action=komarty.php method=post>";
